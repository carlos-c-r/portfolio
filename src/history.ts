for (const elm of document.querySelectorAll('.timelined') as Iterable<HTMLElement>) {
    ...
    elm.style.background = `linear-gradient(to right, var(--accent-fg-disabled-color) ${sp - 2}%, var(--accent-fg-color) ${sp + 2}%, var(--accent-fg-color) ${ep - 2}%, var(--accent-fg-disabled-color) ${ep + 2}%)`;
    //elm.style.background = `linear-gradient(to right, red ${sp - 2}%, green ${sp + 2}%, green ${ep - 2}%, red ${ep + 2}%)`;
